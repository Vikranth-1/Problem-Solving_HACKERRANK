...
int* breakingRecords(int scores_count, int* scores, int* result_count) {
    // Allocate memory for the result array (always 2 integers)
    int* result = malloc(2 * sizeof(int));
    *result_count = 2;
    
    // Initialize with the first game's scores
    int max_score = scores[0];
    int min_score = scores[0];
    int max_breaks = 0;
    int min_breaks = 0;
    
    // Process the remaining games
    for (int i = 1; i < scores_count; i++) {
        ...
        // Note: equal scores don't count as breaks
    }
