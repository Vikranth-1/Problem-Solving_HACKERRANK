...
int divisibleSumPairs(int n, int k, int ar_count, int* ar) {
    ...
    // Initialize remainder counts to 0
    for (int i = 0; i < k; i++) {
        remainder_count[i] = 0;
    }
    ...
    for (int i = 0; i < n; i++) {
        ...
        // The number that would complement this element to make sum divisible by k
        int complement = (k - remainder) % k;
        
        // Add count of elements with the complement remainder from previous elements
