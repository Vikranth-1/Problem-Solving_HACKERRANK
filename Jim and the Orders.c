...
int* jimOrders(int orders_rows, int orders_columns, int** orders, int* result_count) {
    *result_count = orders_rows;
    Order* order_list = malloc(orders_rows * sizeof(Order));
    
    for (int i = 0; i < orders_rows; i++) {
        order_list[i].index = i + 1;
        order_list[i].time = orders[i][0] + orders[i][1];
    }
    
    qsort(order_list, orders_rows, sizeof(Order), compare);
    
    int* result = malloc(orders_rows * sizeof(int));
    for (int i = 0; i < orders_rows; i++) {
        result[i] = order_list[i].index;
    }
