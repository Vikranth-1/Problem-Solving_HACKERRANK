...
int flippingMatrix(int matrix_rows, int matrix_columns, int** matrix) {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            ...
            if (matrix[2 * n - 1 - i][j] > max_val) {
                max_val = matrix[2 * n - 1 - i][j];
            }
            if (matrix[i][2 * n - 1 - j] > max_val) {
                max_val = matrix[i][2 * n - 1 - j];
            }
            if (matrix[2 * n - 1 - i][2 * n - 1 - j] > max_val) {
                max_val = matrix[2 * n - 1 - i][2 * n - 1 - j];
            }
            sum += max_val;
        }
    }
